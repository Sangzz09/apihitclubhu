...
let lastDice = null;         // ← buffer xúc xắc độc lập (cmd 1003 không mang sid)
...
function parseBody(body) {
  ...
  if (cmd === 1008) {
    ...
    const sTB = Number(gi0.B?.tB ?? 0);   // Tài: tổng tiền
    const sTU = Number(gi0.B?.tU ?? 0);   // Tài: số người
    const bTB = Number(gi0.S?.tB ?? 0);   // Xỉu: tổng tiền
    const bTU = Number(gi0.S?.tU ?? 0);   // Xỉu: số người
    const total = sTB + bTB;
...
// ══════════════════════════════════════════════════════════════
// INGEST — xử lý 2 kiểu, giữ sid + xúc xắc đúng phiên
// ══════════════════════════════════════════════════════════════
function ingest(parsed) {
  if (!parsed) return false;

  // ── Kiểu 1: cá cược (có sid) ────────────────────────────────
  if (parsed.kind === "betting") {
    const { sid, sTB, sTU, bTB, bTU, total, ratio } = parsed;

    if (sid === lastSid) {
      // Cùng phiên → cập nhật số liệu cược
    ...
    if (pendingSession) {
      // Nếu lastDice chưa gắn vào pendingSession thì gắn ngay bây giờ
      ...
      lastDice = null; // reset sau khi đã gắn vào phiên
    }
  ...
  // ── Kiểu 2: kết quả xúc xắc (KHÔNG có sid) ─────────────────
  if (parsed.kind === "result") {
    lastDice = parsed.dice; // luôn lưu vào buffer độc lập

    // Nếu pendingSession đang tồn tại → gắn luôn
    if (pendingSession) {
      pendingSession.dice = parsed.dice;
    }
    // Nếu chưa có pendingSession, lastDice sẽ được gắn
    // vào phiên kế tiếp khi 1008 mới về (xem đoạn chốt phiên trên)
    return false;
  }
...
http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/predict" || url.pathname === "/") {
    ...
    // Phiên đã kết thúc lấy từ history[0], phiên đang cược từ pendingSession
    ...
    // Xúc xắc: ưu tiên từ lastLocked, fallback lastDice buffer
    const diceSource = lastLocked?.dice ?? (lastDice ?? null);
    const xucXac = diceSource
      ? [diceSource.d1, diceSource.d2, diceSource.d3]
      : null;

    // Số phiên hiện tại & dự đoán
  ...
  if (url.pathname === "/debug") {
    ...
    res.end(JSON.stringify({
      ...
      last_dice:      lastDice,        // ← hiển thị buffer xúc xắc
  ...
  console.log("✅ Sic-bo prediction port " + PORT);
